...
if (isset($_POST['email']) && $loginsubmit == 1 ) {
  ...
  $LoginRS__query=sprintf("SELECT * FROM %susers WHERE email='%s' AND password='%s'",
  ...
  if ($loginFoundUser>0) {
	...
	$_SESSION['uemail'] = $row_user['email'];
	$_SESSION['ucontact'] = $row_user['contact'];
	$_SESSION['ucompany'] = $row_user['company'];
	$_SESSION['uaddr'] = $row_user['addr'];
	$_SESSION['uphone'] = $row_user['phone'];
	$_SESSION['ulever'] = $row_user['lever'];
...
<div id="container">
    <form ACTION="<?php echo $loginFormAction; ?>" method="POST" name="formlogin" >
		<table width="300px" cellpadding="0" cellspacing="0" border="0" style="margin:10px auto;">
			...
			<tr>
				<td width="70" valign="middle"><div class="light">登陆邮箱：</div></td>
				<td width="85" valign="middle"><input name="email" value="" type="text" class="input1" /></td>
			</tr>
			<tr>   
